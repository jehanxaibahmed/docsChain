...
## Bring up the network if it is not already up.
 ...
 scripts/createChannel.sh $CHANNEL_NAME1 $CLI_DELAY $MAX_RETRY $VERBOSE
 scripts/createChannel.sh $CHANNEL_NAME2 $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
...
if [ "$MODE" == "up" ]; then
  ...
  infoln "Creating channel '${CHANNEL_NAME1}'."
  ...
  infoln "deploying chaincode on channel '${CHANNEL_NAME1}'"
  infoln "deploying chaincode on channel '${CHANNEL_NAME2}'"
else
  printHelp
  exit 1
fi
